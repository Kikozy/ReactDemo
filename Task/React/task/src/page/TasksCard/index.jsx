...
import { Col, Row , Statistic,Card, Switch, Progress, Badge, Modal , Descriptions, Select ,Input, DatePicker ,TimePicker} from 'antd';
...
class TasksCard extends Component{
    constructor(props){
        super(props)
        this.state={
            submitTasksWindowInfo: Object.assign({},this.props.tasksData),
            submitWindowVisible: false,
            delTasksWindowInfo: Object.assign({},this.props.tasksData),
            delTasksWindowVisible: false,
            editTasksWindowInfo: Object.assign({},this.props.tasksData),
            editTasksWindowVisible: false,
            editWindowCache: Object.assign({},this.props.tasksData)
        }
    }
    //打开提交窗口
    ...
    editTasksWindowOk=()=>{
        console.log("提交修改")
        this.props.taskEditFun(this.state.editWindowCache,this.state.editTasksWindowInfo.tasks_name,this.props.tasksIndex)
        this.setState({
            editTasksWindowVisible: false,
            editWindowCache: {}
        })
    ...
    editTasksInfo = e =>{
        console.log(e.target.value)
        let newState = this.state.editWindowCache
        newState.tasks_info = e.target.value
        this.setState({
            editWindowCache: newState
        })
    }
    editTasksStartDate = (value)=>{
        console.log("修改开始日期"+value)
        let newState = this.state.editWindowCache
        newState.tasks_run_start_time = value.valueOf()
        this.setState({
            editWindowCache: newState
        })
    }
    editTasksEndDate = (value)=>{
        console.log("修改结束日期"+value)
        let newState = this.state.editWindowCache
        newState.tasks_run_end_time = value.valueOf()
        this.setState({
            editWindowCache: newState
        })
    }
    editTasksTickTime = (value)=>{
        console.log("修改每日打卡时间"+value)
        let newState = this.state.editWindowCache
        newState.tasks_day_tick_time = value.valueOf()
        this.setState({
            editWindowCache: newState
        })
    }
    editTasksTickCout = (e) =>{
        console.log(e.target.value)
        let newState = this.state.editWindowCache
        newState.tasks_count = e.target.value
        this.setState({
            editWindowCache: newState
        })
    }
    // //转换时间戳
    // changeNumTime(type,value){
    //     let numTime = new Date(value)
    //     if(type == "日期")
    //         return `${numTime.getFullYear()}-${numTime.getMonth()+1}-${numTime.getDate()}`
    //     else
    //         return `${numTime.getHours()}:${numTime.getMinutes()}:${numTime.getSeconds()}`
    // }
    render(){
        return(
            <Fragment>
                                ...
                                return(
                                    <Descriptions title="" bordered column={{ xxl: 1, xl: 1, lg: 1, md: 1, sm: 1, xs: 1 }}>
                                        ...
                                        <Descriptions.Item label="任务类型">
                                            <Select value={this.state.editWindowCache.tasks_type} style={{width: "100%"}} onChange={this.editTasksWindowEditType}>
                            ...
                            case "循环计次任务":
                                return(
                                    <Descriptions title="" bordered column={{ xxl: 1, xl: 1, lg: 1, md: 1, sm: 1, xs: 1 }}>
                                        ...
                                        <Descriptions.Item label="任务描述"><TextArea onChange={this.editTasksInfo} value={this.state.editWindowCache.tasks_info} placeholder={this.state.editWindowCache.tasks_info}></TextArea></Descriptions.Item>
                                        <Descriptions.Item label="目标次数"><Input onChange={this.editTasksTickCout} value={this.state.editWindowCache.tasks_count} /></Descriptions.Item>
                                        <Descriptions.Item label="每日签到截止时间"><TimePicker onChange={this.editTasksTickTime}/></Descriptions.Item>
                                        <Descriptions.Item label="开始日期"><DatePicker onChange={this.editTasksStartDate}/></Descriptions.Item>
                                        <Descriptions.Item label="结束日期"><DatePicker onChange={this.editTasksEndDate}/></Descriptions.Item>
                                    </Descriptions>
                                )
                            default:
                                return(
                                    ...
                                    return(
                                        ...
                                        >
                                            ...
                                            <Col span={24}></Col>
                                                ...
                                                <Col span={12}><Countdown title="今日签到时间" value={new Date(new Date(this.state.editWindowCache.tasks_day_tick_time))}/></Col>
                                                <Col span={24}><Progress strokeColor={{from: '#108ee9',to: '#87d068'}} percent={10} status="active"/></Col>
                                            </Row>
                                        </Card>
                                    )
